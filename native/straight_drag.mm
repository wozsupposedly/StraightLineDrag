...
std::atomic_bool g_snapLocked{false};
...
Point MaybeSnapPoint(Point point, bool shiftDown) {
  if (!g_config.requireShiftForSnap) {
    return point;
  }
...
void WaitForPhysicalModifierRelease();
...
CGEventRef EventTapCallback(CGEventTapProxy, CGEventType type, CGEventRef event, void*) {
  ...
  if (type == kCGEventFlagsChanged) {
    ...
    if (g_hotkeyActive.load()) {
      const bool hotkeyKeyStillDown = g_hotkeyKeyDown.load();
...
void WaitForPhysicalModifierRelease() {
  for (int attempt = 0; attempt < 40; ++attempt) {
    if (AreRequiredModifiersReleased()) {
      return;
    }
...
LRESULT CALLBACK KeyboardHookProc(int nCode, WPARAM wParam, LPARAM lParam) {
  ...
  if (g_hotkeyActive.load()) {
    const bool isManagedModifier =
        hookData->vkCode == VK_LCONTROL || hookData->vkCode == VK_RCONTROL || hookData->vkCode == VK_CONTROL ||
        hookData->vkCode == VK_LMENU || hookData->vkCode == VK_RMENU || hookData->vkCode == VK_MENU ||
        hookData->vkCode == VK_LSHIFT || hookData->vkCode == VK_RSHIFT || hookData->vkCode == VK_SHIFT;
    const bool hotkeyKeyStillDown = g_hotkeyKeyDown.load();
    FinishHotkeyIfReleased();
    if (isManagedModifier && hotkeyKeyStillDown) {
      return 1;
    }
  }
